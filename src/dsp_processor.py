...
class DSPProcessor:
    def __init__(self):
        ...
        self.mel_gain = ExpFilter(np.tile(1e-1, config.N_FFT_BINS), alpha_decay=0.01, alpha_rise=0.99)
        self.mel_smoothing = ExpFilter(np.tile(1e-1, config.N_FFT_BINS), alpha_decay=0.5, alpha_rise=0.99)
        self.gain = ExpFilter(np.tile(0.01, config.N_FFT_BINS), alpha_decay=0.001, alpha_rise=0.99)
        self.fft_window = np.hamming(int(config.RATE / config.FPS) * config.N_ROLLING_HISTORY)
        self.y_roll = np.random.rand(config.N_ROLLING_HISTORY, int(config.RATE / config.FPS)) / 1e16
    ...
    def get_level(self, y_data):
        ...
        if vol < config.MIN_VOLUME_THRESHOLD:
            ...
            return np.zeros(config.N_FFT_BINS)  # Return zeros if volume is too low
